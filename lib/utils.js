...
utils.flatten = function(collection, index, order, depth, remove) {
  order = order || 'shallow';
  remove = utils.isSet(remove) ? remove : true;

  var result = utils.copy(utils.isArray(collection) ? collection : [collection]);

  if(utils.isSet(depth) && depth === 0) {
    if(remove) {
  ...
  for(var i = 0; i < result.length; i++) {
    ...
    if(result[i][index]) {
      if(order === 'shallow') {
        result = result.concat(utils.flatten(result[i][index], index, order, 0, false));
      } else if(order === 'deep') {
        var end = result.splice(i + 1, result.length - (i + 1));
        result = result.concat(utils.flatten(result[i][index], index, order, depth-1));
